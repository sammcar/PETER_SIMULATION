...
class JointPositionPublisher(Node):
    def __init__(self):
        ...
        self.ticker = self.create_timer(0.1, self.ticker_callback)
    ...
    #     """
    #     global site_now, site_expect  # Assuming these are defined globally

    #     while True:
    #         rclpy.spin_once(self, timeout_sec=0.01)
    #         self.get_logger().info("site_now[0][0]: "+str(site_now[leg][0]))
    ...
    def step_forward(self, step):
        ...
        while step > 0:  # Repeat until the specified number of steps is complete
            ...
            if site_now[2][1] == Y_START:  # PASO CON LA IZQUIERDA
                ...
                self.set_site(2, X_DEFAULT + X_OFFSET, Y_START, Z_UP)  # Lift leg 2
                self.wait_all_reach()
                self.set_site(2, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_UP)  # Move leg 2 forward
                self.wait_all_reach()
                self.set_site(2, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_DEFAULT)  # Lower leg 2
                self.wait_all_reach()
                ...
                self.set_site(0, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_UP)  # Lift leg 1
                self.wait_all_reach()
                self.set_site(0, X_DEFAULT + X_OFFSET, Y_START, Z_UP)  # Move leg 1 back
                self.wait_all_reach()
                self.set_site(0, X_DEFAULT + X_OFFSET, Y_START, Z_DEFAULT)  # Lower leg 1
                self.wait_all_reach()
            else:  # PASO CON LA DERECHA
                # Alternate step: Lift and move legs 0 and 3 forward
                self.set_site(3, X_DEFAULT + X_OFFSET, Y_START, Z_UP)  # Lift leg 3
                self.wait_all_reach()
                self.set_site(3, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_UP)  # Move leg 3 forward
                self.wait_all_reach()
                self.set_site(3, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_DEFAULT)  # Lower leg 3
                self.wait_all_reach()
                ...
                self.set_site(1, X_DEFAULT + X_OFFSET, Y_START + 2 * Y_STEP, Z_UP)  # Lift leg 1
                self.wait_all_reach()
                self.set_site(1, X_DEFAULT + X_OFFSET, Y_START, Z_UP)  # Move leg 1 back
                self.wait_all_reach()
                self.set_site(1, X_DEFAULT + X_OFFSET, Y_START, Z_DEFAULT)  # Lower leg 1
                self.wait_all_reach()
    ...
    def cmd_vel_callback(self, msg):
        ...
        if self.state == 'X':  # Modo omnidireccional
            ...
            if self.linear_x > 0 and not self.already:
                self.get_logger().info("Activando STEP FORWARD")
                self.already = True
                self.step_forward(1)
    
    def ticker_callback(self):
        self.get_logger().info("Running TICKER")
        """
        ...
        for i in range(4):  # imax = 4
            ...
            if i in (0, 2):  # Adjust alpha for legs 0 and 2
                alpha = 180 - alpha
    ...
    def setFrontStep(self):
        ...
        self.set_site(3, X_DEFAULT + X_OFFSET, Y_START + Y_STEP, Z_BOOT)

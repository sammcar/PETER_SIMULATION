...
try:
   ...
   #------------------------E S T I M U L O -------------------------------------#
    ...
    #------------------------- TERRAIN CHANGER -----------------------------
        ...
        if (inclinado > Uprob_inclinado): pitch = 8
        else: pitch = 0

        if  ((plano > Uprob_plano) and estancado) and not terrainchanger:
        ...
        if (command_char.get(modo) != None):
            ...
            if (time.time()-modo_time > 1.0 and modo_ant != cmd):
                modo_ant = cmd
                ser.write(cmd.encode())   # Envía el comando
                print(f"Enviado: {cmd!r}")
                modo_time = time.time()

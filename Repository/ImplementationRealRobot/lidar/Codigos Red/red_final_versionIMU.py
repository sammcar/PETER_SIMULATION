...
# === IMU SHM (3 floats: [stuck, flat, inclined]) ===
IMU_SHM_PATH = "/dev/shm/imu_activity"
IMU_COUNT    = 3

# ====== TERRAIN ======
STUCK_DEBOUNCE_S     = 3.0   # estancado debe durar al menos 3 s para disparar
MIN_HOLD_ROCOSO_S    = 60.0  # mantener rocoso mínimo 10 s antes de poder salir

# --- Timers para PITCH por 'inclinado' ---
PITCH_HOLD_S = 35
.0
...
def shm_setup_vec(path: str, count: int):
    """Create/open a float32 vector SHM with seqlock (8B + count*4). Returns (fd, mm)."""
    size = 8 + count * 4
    fd = os.open(path, os.O_RDWR | os.O_CREAT, 0o666)
    os.ftruncate(fd, size)
    mm = mmap.mmap(fd, size, access=mmap.ACCESS_WRITE)
    struct.pack_into("Q", mm, 0, 0)  # seq = 0 (even = stable)
    return fd, mm

def shm_write_vec(mm, vec: np.ndarray, expected_len: int):
    """Write float32 vector with seqlock."""
    assert vec.size == expected_len, f"expected {expected_len} floats, got {vec.size}"
    seq = struct.unpack_from("Q", mm, 0)[0]
    seq = seq + 1 if seq % 2 == 0 else seq + 2  # enter (odd)
    struct.pack_into("Q", mm, 0, seq)
    mm.seek(8)
    mm.write(vec.astype(np.float32, copy=False).tobytes(order="C"))
    struct.pack_into("Q", mm, 0, seq + 1)       # leave (even)
    mm.flush()
...
pitch_hold_until_mono = 0.0  # instante (monotonic) hasta el que debe mantenerse a 20
...
if USE_IMU:
    try:
        imu_fd, imu_mm = shm_setup_vec(IMU_SHM_PATH, IMU_COUNT)
        print("[IMU] SHM publisher ready at /dev/shm/imu_activity (stuck, flat, inclined)")
    except Exception as e:
        print(f"[IMU] SHM init error: {e}")
        imu_mm = None
else: 
    print("[IMU] SHM deshabilitado; uso simulado")
...
try:
    while not stop_evt.is_set():
        ...
        #     got_update = False
        #     try:
        #         # Drenar todo lo disponible y quedarnos con la última
        #         while True:
        #             probs, stuck = mlp_queue.get_nowait()
        #             inclinado = float(probs[1])
        #             plano     = float(probs[0])
        #             estancado = bool(stuck)
        #             last_mlp_ts = time.time()
        #             got_update = True
        #     except Empty:
        #         pass  # no llegó nada en este ciclo
        #     except Exception as e:
        #         if DEBUG:
        #             print(f"[MLP] error leyendo cola: {e}")

        #     # (Opcional) Si está “stale”, puedes forzar valores seguros o sólo advertir
        #     if not got_update and (time.time() - last_mlp_ts) > MLP_STALE_SEC:
        #         if DEBUG:
        #             print("[MLP] muestras IMU desactualizadas (>1.5 s)")
        # else:
        #     pass
   ...
   #------------------------E S T I M U L O -------------------------------------#
        ...
        z[1, 1] = z[1, 0] + (dt / tau) * (-z[1, 0] + (A*2 * max(0, (pitch - Upitch))**2) / (SigmaIMU**2 + (-z[1,0] + pitch - Upitch)**2))
    ...
    #------------------------- TERRAIN CHANGER -----------------------------
        ...
        if now_mono < pitch_hold_until_mono:
            # Todavía dentro de la ventana de retención
            pitch = 20
        else:
            if inclinado > Uprob_inclinado:
                # Disparo: activa y congela 12 s
                pitch_hold_until_mono = now_mono + PITCH_HOLD_S
                pitch = 20
            else:
                pitch = 0
        ...
        # 2) SI YA ESTÁ EN ROCOSO: mantener mínimo 60 s; cumplido el mínimo, salir si estancado=False
        if terrainchanger:
        ...
        if USE_IMU:
            # ====== PUBLISH IMU (3 channels: stuck, flat, inclined) ======
            try:
                if imu_mm is not None:
                    stuck_bin    = float(estancado)
                    flat_prob    = float(plano)      # from MLP (0..1)
                    incl_prob    = float(inclinado)  # from MLP (0..1)
                    imu_vec = np.array([stuck_bin, flat_prob, incl_prob], dtype=np.float32)
                    shm_write_vec(imu_mm, imu_vec, IMU_COUNT)
            except Exception as e:
                if DEBUG: print(f"[IMU-SHM] write error: {e}")
        else:
            pass

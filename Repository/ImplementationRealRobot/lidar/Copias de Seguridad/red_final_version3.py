...
# (opcional) tu parser/MLP, si quieres procesar líneas
...
# ====== MODO SIMULACIÓN ======
USE_LIDAR_SHM   = False   # si False, usa LIDAR simulado
USE_VISION_SHM  = False # si False, usa detecciones simuladas
USE_IMU = True       # si False, usa IMU simulado
...
class TerrainFSM:
    def __init__(self, *, t_set=2, t_clear_stuck=10, t_clear_flat=10,
                 hold_max=10.0, cool_down=0.5, safety=0.6):
        self.state = False               # False = liso, True = rocoso
        self.t_set = float(t_set)
        self.t_clear_stuck = float(t_clear_stuck)
        self.t_clear_flat = float(t_clear_flat)
        ...
        # temporizadores de dwell
        self._rocky_since = now
        self._stuck_false_since = now
        self._flat_since = now

        # memorias de condición
        self._was_rocky = False
        self._was_stuck_false = None
        self._was_flat = None

    def update(self, *, plano, estancado, pitch, accel_std, Uprob_plano, Upitch, Usigma_az):
        now = time.time()

        # --- condiciones atómicas ---
        rocky_cond = (plano > Uprob_plano and estancado) or (accel_std > Usigma_az)
        stuck_false = (estancado is False)
        flat_cond = (plano <= Uprob_plano) and (pitch < Upitch) and (accel_std < Usigma_az * self.safety)

        # --- dwell de rocky_cond ---
        if rocky_cond != self._was_rocky:
            self._was_rocky = rocky_cond
            self._rocky_since = now
        rocky_dwell = now - self._rocky_since

        # --- dwell de stuck_false (prioridad de salida) ---
        if self._was_stuck_false is None or stuck_false != self._was_stuck_false:
            self._was_stuck_false = stuck_false
            self._stuck_false_since = now
        stuck_false_dwell = now - self._stuck_false_since

        # --- dwell de flat_cond (salida secundaria) ---
        if self._was_flat is None or flat_cond != self._was_flat:
            self._was_flat = flat_cond
            self._flat_since = now
        flat_dwell = now - self._flat_since

        # --- lógica de flips con cooldown ---
        can_flip = (now - self._last_flip) >= self.cool_down

        if not self.state:
            # Estado actual: LISO → ¿entrar a ROCOSO?
            if can_flip and rocky_cond and rocky_dwell >= self.t_set:
                self.state = True
                self._last_flip = now
                self._entered_at = now
        else:
            # Estado actual: ROCOSO → ¿salir a LISO?
            elapsed = now - self._entered_at

            # (A) prioridad: estancado == False suficiente tiempo
            if can_flip and stuck_false and stuck_false_dwell >= self.t_clear_stuck:
                self.state = False
                self._last_flip = now

            # (B) plano y suave suficiente tiempo (fallback)
            elif can_flip and flat_cond and flat_dwell >= self.t_clear_flat:
                self.state = False
                self._last_flip = now

            # (C) “freno de mano” máximo (seguridad)
            elif elapsed >= self.hold_max and can_flip:
                self.state = False
                self._last_flip = now

        # Valor sugerido de std_dev_accel_z (imitando tu código)
        std_override = 9.0 if self.state else 0.0
        return self.state, std_override
...
# --- define estos antes para evitar NameError en finally ---
...
command_char = { #MAPA del Serial 
    ...
    "Desp. Izquierda": 'j', #Girar izquierda cuadrupedo
    "Desp. Derecha":  'l', #Girar Derecha cuadrupedo
    "A": 'a',
    "B": 'b',
    "C": 'c',
}
...
# === Terrain FSM ===
terrain_fsm = TerrainFSM(
    t_set=1.5,          # entrar a rocoso
    t_clear_stuck=1.0,  # salir si estancado=False (prioridad)
    t_clear_flat=2.5,   # salir si plano & suave
    hold_max=10.0,      # igual que tu lógica original
    cool_down=0.5,      # antirrebote
    safety=0.6          # accel_std < 0.6*Usigma_az para considerar “suave”
)
...
try:
    ...
    # corre tu controlador en el hilo principal (o en otro hilo si prefieres)
...
try:
    while not stop_evt.is_set():
        ...
        # ranges debe estar en metros.
        #  
        # === LIDAR ===
        if (t0 - time_lidar_reset) > 1:
            lidar_points = np.full(NUM_BINS, np.inf, dtype=np.float32)
            time_lidar_reset = time.time()
   ...
   #------------------------E S T I M U L O -------------------------------------#
    ...
    #------------------------- TERRAIN CHANGER -----------------------------
        ...
        # ------------------------- TERRAIN CHANGER (FSM con prioridad a 'estancado == False') -----------------------------
        # Tu mapeo de inclinación (lo mantengo):
        pitch = 8 if (inclinado > Uprob_inclinado) else 0

        # Calcula estado con histéresis:
        terrainchanger, std_dev_accel_z = terrain_fsm.update(
            plano=plano,
            estancado=estancado,
            pitch=pitch,
            accel_std=accel_std,           # o 'accel_std2' si prefieres
            Uprob_plano=Uprob_plano,
            Upitch=Upitch,
            Usigma_az=Usigma_az
        )

        print("terrainchanger: ", str(terrainchanger))
        ...
        if (command_char.get(modo) != None):
            ...
            if (modo_ant != cmd):
                 t0 = time.time()
                 while time.time() - t0 < 2.5:   # 0.5 segundos
                    modo_ant = cmd

...
static const char* dir_name(GaitDir d) {
    switch (d) {
        case GAIT_FORWARD:    return "ADELANTE";
        case GAIT_BACKWARD:   return "ATRAS";
        case GAIT_LEFT:       return "IZQUIERDA";
        case GAIT_RIGHT:      return "DERECHA";
        case GAIT_FWD_LEFT:   return "ADELANTE+IZQ";
        case GAIT_FWD_RIGHT:  return "ADELANTE+DER";
        case GAIT_BCK_LEFT:   return "ATRAS+IZQ";
        case GAIT_BCK_RIGHT:  return "ATRAS+DER";
        default:              return "STOP";
    }
}
...
static void handle_teleop(char key) {
    GaitDir dir;
    switch (key) {
        case 'i': dir = GAIT_FORWARD;    break;
        case ',': dir = GAIT_BACKWARD;   break;
        case 'j': dir = GAIT_LEFT;       break;
        case 'l': dir = GAIT_RIGHT;      break;
        case 'k': dir = GAIT_STOP;       break;
        case 'c':
            ...
            return;
        case 'u': dir = GAIT_FWD_LEFT;   break;
        case 'o': dir = GAIT_FWD_RIGHT;  break;
        case 'm': dir = GAIT_BCK_LEFT;   break;
        case '.': dir = GAIT_BCK_RIGHT;  break;
        default:  return;
    }
    gait_set_dir(dir);
    Serial.print("CMD=");   Serial.print(key);
    Serial.print(" DIR=");  Serial.println(dir_name(dir));
}
...
static void handle_joint_cmd(const String& line) {
    ...
    if (sep1 < 0 || sep2 < 0) { Serial.println("ERROR formato: usa leg,joint,angulo"); return; }

    int   leg   = line.substring(0, sep1).toInt();
    int   joint = line.substring(sep1 + 1, sep2).toInt();
    float ang   = line.substring(sep2 + 1).toFloat();

    if (leg < 0 || leg > 3)     { Serial.println("ERROR pata fuera de rango (0-3)");        return; }
    if (joint < 0 || joint > 2) { Serial.println("ERROR articulacion fuera de rango (0-2)"); return; }
    ...
    Serial.print("CMD=");     Serial.print(line);
    Serial.print(" pata=");   Serial.print(leg_name(leg));
    Serial.print(" art=");    Serial.print(joint);
    Serial.print(" canal=");  Serial.print(CALIB[leg][joint].channel);
    Serial.print(" ang=");    Serial.println(ang, 1);
}

static void procesar_serial(const String& line) {
    if (line.length() == 0) return;

    if (line.length() == 1) {
        handle_teleop(line[0]);
        return;
    }

    if (line.indexOf(',') >= 0) {
        handle_joint_cmd(line);
        return;
    }
...
void setup() {
    ...
    if (USE_IMU) {
        ...
        if (imu_ok()) {
            ...
            Serial.println("AVISO: MPU-6050 no detectado (verifica conexion I2C en 0x68)");
        }
    } else {

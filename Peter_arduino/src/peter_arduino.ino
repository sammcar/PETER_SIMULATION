...
static void handle_teleop(char key) {
  ...
  // DEBUG RR: silencia PCA9685 ch9 y ch10 (RL femur/tibia) y testea RR
  // Si RR gira → el PCA9685 estaba pisando GPIO 9/10 (problema de cableado)
  case '7': {
    pca.setPWM(9,  0, 4096); // ch9  full LOW – RL fémur silenciado
    pca.setPWM(10, 0, 4096); // ch10 full LOW – RL tibia silenciado
    delay(50);
    motor_set(0, 0, 100, 0);
    Serial.println("DEBUG: PCA ch9+ch10 silenciados -> TEST RR");
    return;
  }
  // DEBUG RR: GPIO raw sin LEDC – bypass total de analogWrite
  // Si RR se mueve → LEDC corrupto en modo H. Si no → hardware.
  case '8': {
    motors_stop();
    ...
    delay(400);
    digitalWrite(9, LOW);
    Serial.println("DEBUG: GPIO9 raw HIGH 400ms -> RR deberia jalarse brevemente");
    return;
  }
  }
